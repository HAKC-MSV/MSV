...
public:
    ...
    bool hasSVFValue(const Value* value);
    ...
    inline bool hasSVFBasicBlock(const BasicBlock* bb) const
    {
        LLVMBB2SVFBBMap::const_iterator it = LLVMBB2SVFBB.find(bb);
        return it != LLVMBB2SVFBB.end();
    }
    ...
    inline bool hasSVFInstruction(const Instruction* inst) const
    {
        LLVMInst2SVFInstMap::const_iterator it = LLVMInst2SVFInst.find(inst);
        return it != LLVMInst2SVFInst.end();
    }
    ...
    inline bool hasSVFArgument(const Argument* arg) const
    {
        LLVMArgument2SVFArgumentMap::const_iterator it = LLVMArgument2SVFArgument.find(arg);
        return it != LLVMArgument2SVFArgument.end();
    }
    ...
    inline bool hasSVFGlobalValue(const GlobalValue* g) const
    {
        LLVMConst2SVFConstMap::const_iterator it = LLVMConst2SVFConst.find(g);
        if (it != LLVMConst2SVFConst.end())
        {
            return SVFUtil::isa<SVFGlobalValue>(it->second);
        }
        return false;
    }
