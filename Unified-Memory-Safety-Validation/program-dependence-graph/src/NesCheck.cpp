...
namespace NesCheck
{
  ...
  private:
    ...
    BasicBlock *getTrapBB(Instruction *CurrInst)
    {
      ...
      llvm::Function *F = Intrinsic::getDeclaration(Fn->getParent(), Intrinsic::trap);
      CallInst *TrapCall = Builder->CreateCall(F->getFunctionType(), F);
        ...
        if (II->getType()->isPointerTy())
        {
          ...
          if (varinfo != NULL)
          {
                                                     ...
                                                     (isa<Instruction>(varinfo->size) && ((Instruction *)varinfo->size)->getParent() != II->getParent())))
            {
              ...
              LoadInst *loadsize = Builder->CreateLoad(varinfo->explicitSizeVariable->getType(), varinfo->explicitSizeVariable);
    ...
    bool rewriteCallSite(Instruction *Call)
    {
      ...
      CallBase::op_iterator AI = Call->op_begin(), AE = Call->op_end() - 1;
