...
std::unordered_set<pdg::Node *> &pdg::GenericGraph::findNodesReachedByEdges(pdg::Node &src, std::set<EdgeType> &edge_types, bool is_backward)
{
  std::unordered_set<Node *> *ret = new std::unordered_set<Node *>();
  ...
  while (!node_queue.empty()){
    ...
    for (auto edge_type : edge_types) {
      auto &neighbors = is_backward ? src.getInNeighborsWithDepType(edge_type) : src.getOutNeighborsWithDepType(edge_type);
      for (auto neighbor : neighbors) {
        if (ret->find(neighbor) != ret->end())
          continue;
        ret->insert(neighbor);
...
{
  errs() << "PDG: Building global variables...\n";
  ...
  errs() << "PDG: Building call sites...\n";
  // handle call sites - OPTIMIZATION: Skip building actual trees to reduce memory
  // Actual tree building for every call site can cause exponential memory growth with large structs
  // The data dependency graph should still work without these trees
  for (auto &F : M)
  {
    if (F.isDeclaration() || F.empty())
      continue;
...
void pdg::ProgramGraph::bindDITypeToNodes(Module &M)
{
  for (auto &F : M)
  {
    ...
    {
      auto addr = dbg_declare_inst->getAddress();
...
DIType *pdg::ProgramGraph::computeNodeDIType(Node &n)
{
  ...
  if (GetElementPtrInst *gep = dyn_cast<GetElementPtrInst>(val))
  {
    ...
    if (auto dict = dyn_cast<DICompositeType>(base_addr_lowest_di_type))
    {
      ...
      {
        // Only interested in DW_TAG_member
        auto *member = dyn_cast<DIDerivedType>(di_node_arr[i]);
        if (!member)
          continue;

        if (member->getTag() != dwarf::DW_TAG_member)
          continue; // skip inheritance/methods/etc.

        DIType *field_di_type = member->getBaseType();
        if (!field_di_type) {
          errs() << "Missing base type for field in " 
                            << dict->getName() << "\n";
          continue;
        }

        // assert(field_di_type != nullptr && "fail to retrive field di type (computeNodeDIType)");
        // Instead of assuming array index maps to field index,
        // compare GEP offset vs DWARF field offset:
