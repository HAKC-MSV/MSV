...
namespace pdg
{
  ...
  {
  public:
    // using a hash set to store edges, fast inserting and searching
    using EdgeSet = std::unordered_set<Edge, Edge::HashFunction>;
    ...
    NodeSet &getInNeighbors();
    NodeSet &getInNeighborsWithDepType(EdgeType edge_type);
    NodeSet &getOutNeighbors();
    NodeSet &getOutNeighborsWithDepType(EdgeType edge_type);
    bool hasInNeighborWithEdgeType(Node &n, EdgeType edge_type);
    bool hasOutNeighborWithEdgeType(Node &n, EdgeType edge_type);
    NodeSet &getNeighborsWithDepType(std::set<EdgeType> edge_types);
    bool isAddrVarNode();
  ...
  protected:
    ...
    NodeSet _in_neighbors;
    NodeSet _out_neighbors;
    NodeSet _in_neighbors_with_edge_type[static_cast<int>(EdgeType::VAL_DEP)+1];
    NodeSet _out_neighbors_with_edge_type[static_cast<int>(EdgeType::VAL_DEP)+1];
