	...
	private:
		...
		Value *getSizeForValue(Value *v){
			...
			if (ObjSizeEval->knownSize(SizeOffset)){
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\tUsing Size from ObjSizeEval = " << *(SizeOffset.first) << "\n";);
				size = SizeOffset.first;
			}
			else{
				Type *t = v->getType();
				if (!isa<Function>(v))
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\tUsing manual Size (ObjSizeEval failed) for " << *v << " - type:" << *t << "\n";);
				else
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\tUsing manual Size (ObjSizeEval failed) for " << ((Function *)v)->getName() << " - type:" << *t << "\n";);

				if (t->isPointerTy())
					t = ((PointerType *)t)->getElementType();
				if (ArrayType *arrT = dyn_cast<ArrayType>(t)){
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\t\tarray[" << arrT->getNumElements() << " x " << *(arrT->getElementType()) << "]\n";);
					...
					UMS_DEBUG(DEBUG_SWITCH, errs() << "Newly size" << *size << "\n";);
				}
				else if (isa<FunctionType>(t)){
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\t\t" << *t << " is a FunctionType\n";);
					...
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\t\t" << *t << " is a CallInst/InvokeInst returning a pointer type\n";);
					// if this is a call to an unininstrumented function that returns a pointer, we don't have info
					size = UnknownSizeConstInt;
				} else if (isa<Argument>(v)) {
					// Try to get the size from SVF
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\t\t" << *v << " is an Argument\n";);
					size = ConstantInt::get(MySizeType, ptaw->getSizeForValue(*v, CurrentDL));
				}
				else{
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\t\t" << *t << " is not a special-case type for manual sizing\n";);
					// last attempt at getting size (for structs)
					if (t->isSized())
						size = ConstantInt::get(MySizeType, CurrentDL->getTypeAllocSize(t));
				}
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\tManual Size is " << *size << "\n";);
			}
		  return size;
		}
		...
		Value *getOffsetForGEPInst(GetElementPtrInst *GEPInstr) {
			...
			if (ObjSizeEval->knownOffset(SizeOffset)) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\tUsing Offset from ObjSizeEval = " << *(SizeOffset.second) << "\n";);
			...
			if (GEPInstr->accumulateConstantOffset(*CurrentDL, Off)) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\tUsing Offset from GEP.accumulateConstantOffset() = " << Off << "\n";);
				return ConstantInt::get(MySizeType, Off);
			}

			// 3) Manual fallback
			uint64_t typeStoreSize = CurrentDL->getTypeStoreSize(GEPInstr->getResultElementType());
			UMS_DEBUG(DEBUG_SWITCH, errs() << "\tSize of type of Ptr = " << typeStoreSize << "\n";);
			...
			for (unsigned i = 1; i < GEPInstr->getNumOperands(); ++i) {
				...
				if (cVal == -1) {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\tIndex " << i << " not a single constant. Returning unknown.\n";);
					return UnknownSizeConstInt;
				}
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\tResolved index " << i << " to constant " << cVal << "\n";);
				totalOffset += (cVal * typeStoreSize);
			}

			Constant *OffsetConst = ConstantInt::get(MySizeType, totalOffset);
			UMS_DEBUG(DEBUG_SWITCH, errs() << "\tUsing Offset from manual evaluation = " << *OffsetConst << "\n";);
			return OffsetConst;
		}
		...
		void logInstructionStart(Instruction *I) {
			...
			ss << (const void *)I;
			if (I != nullptr){
				UMS_DEBUG(DEBUG_SWITCH, errs() << BLUE << "[" << ss.str() << "] " << NORMAL;);
			}
		}
		...
		bool processAllocaInst(AllocaInst *II) {
			...
			UMS_DEBUG(DEBUG_SWITCH, errs() << "(+) " << *II << "\t" << DETAIL << " // {";);
			if (isArray)
				UMS_DEBUG(DEBUG_SWITCH, errs() << " array[" << *(II->getArraySize()) << "]";);
			UMS_DEBUG(DEBUG_SWITCH, errs() << " (" << *(II->getAllocatedType()) << ") "
				<< "}" << NORMAL << "\n";);
		...
		bool processCallInst(CallInst *II) {
			if (II->getCalledFunction() != NULL) {
				...
				if (funcName.contains("malloc") && II->getCalledFunction()->arg_size() == 1) {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "(M) " << *II << "\n";);
		...
		bool processStoreInst(StoreInst *II) {
			...
			if (!isa<Function>(valoperand))
				UMS_DEBUG(DEBUG_SWITCH, errs() << "(~) " << *II << "\t" << DETAIL << " // {" << *valoperand
					<< " -> " << *(II->getPointerOperand()) << *(II->getPointerOperandType()) << " }" << NORMAL << "\n";);
			else
				UMS_DEBUG(DEBUG_SWITCH, errs() << "(~) " << *II << "\t" << DETAIL << " // {" << ((Function *)valoperand)->getName()
			...
			if (valoperand->getType()->isPointerTy()) {
				...
				// if we're storing a non-pointer, ensure the pointer location has a size
				Value *ptroperand = II->getPointerOperand();
				UnifiedMemSafe::VariableInfo *varinfo = TheState.GetPointerVariableInfo(ptroperand);
				if (!varinfo) {
					TheState.SetSizeForPointerVariable(ptroperand, getSizeForValue(ptroperand));
		...
		bool processLoadInst(LoadInst *II) {
			...
			UMS_DEBUG(DEBUG_SWITCH, errs() << " (~) " << *II << "\n";);
			if (II->getType()->isPointerTy()){
				//UMS_DEBUG(DEBUG_SWITCH, errs() << " Load Instruction Type: " << *(II->getType()) << "\n";);
				Value *ptroperand = II->getPointerOperand();
				UnifiedMemSafe::VariableInfo *varinfo = TheState.GetPointerVariableInfo(ptroperand);
				...
				if (varinfo != NULL){
					...
					//UMS_DEBUG(DEBUG_SWITCH, errs() << PtrTypeToString(varinfo->classification) << " \n";);
					TheState.ClassifyPointerVariable(II, varinfo->classification);
		...
		bool processGetElementPtrInst(GetElementPtrInst *II) {
			...
			UMS_DEBUG(DEBUG_SWITCH, errs() << "(*) " << *II << "\t" << DETAIL << " // {" << *(Ptr) << " (" << *(II->getPointerOperandType()) << ") | " << *(II->getType()) << " -> " << *(II->getResultElementType()) << " }" << NORMAL << "\n";);

			UMS_DEBUG(DEBUG_SWITCH, errs() << "\tIndices = " << (II->getNumOperands() - 1) << ": ";);
			UMS_DEBUG(DEBUG_SWITCH, errs() << "\t";);
			for (unsigned int operd = 1; operd < II->getNumOperands(); operd++)
				UMS_DEBUG(DEBUG_SWITCH, errs() << *(II->getOperand(operd)) << " ; ";);
			UMS_DEBUG(DEBUG_SWITCH, errs() << "\n";);
			...
			if (II->getResultElementType()->isPointerTy()){
				...
				UnifiedMemSafe::VariableInfo *varinfo = TheState.GetPointerVariableInfo(valoperand);
				//UMS_DEBUG(DEBUG_SWITCH, errs() << "\tvarinfo at " << varinfo << "\n ";);
				...
				if (varinfo != NULL) {
					...
					if (auto *constInt = dyn_cast<ConstantInt>(otherSize)) {
						if (constInt->isZero()) {
							...
							if (elementType->isArrayTy()) {
								...
								otherSize = getSizeForValue(valoperand);
							}
					...
					if (!(II->hasAllZeroIndices())) {
						...
						if (varinfo->size->getType() != Offset->getType()) {
							UMS_DEBUG(DEBUG_SWITCH, errs() << RED << "\t!!! varinfo->size->getType() (" << *(varinfo->size->getType()) << ") != Offset->getType() (" << *(Offset->getType()) << ")\n"
								<< NORMAL;);
							UMS_DEBUG(DEBUG_SWITCH, errs() << GREEN <<"\t Converting Offset and calculating new size\n" << NORMAL;);
						}
						...
						if (offset && othersize) {
							...
							if (!offset->getType()->isIntegerTy(64)) {
								offset = ConstantExpr::getZExtOrBitCast(offset, Type::getInt64Ty(II->getContext()));
							}
							if (!othersize->getType()->isIntegerTy(64)) {
								othersize = ConstantExpr::getZExtOrBitCast(othersize, Type::getInt64Ty(II->getContext()));
							}
		...
		bool processCastInst(CastInst *II) {
			...
			if (!srcT || !dstT) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << "Invalid source or destination type for CastInst: " << *II << "\n";);
				return false;
			}
			if (!II->getOperand(0)) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << "Null operand for instruction: " << *II << "\n";);
				return false;
			}
			...
			if (!varinfo) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\tPointerVariableInfo not found for operand: " << *(II->getOperand(0)) << "\n\tCreating and Recheck\n";);
			...
			if (isCastOfInterest(srcT, dstT, innerSrcT, innerDstT)) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << *II << "  Cast of Interest!\n";);
				...
				if (LoadInst *III = dyn_cast_or_null<LoadInst>(II->getOperand(0))) {
					...
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\tIgnored classification of variable since we have no operand\n";);
				}
		...
		void logCastInstructionDetails(CastInst *II) {
			Type *srcT = II->getSrcTy();
			Type *dstT = II->getDestTy();
			UMS_DEBUG(DEBUG_SWITCH, errs() << "(>) " << *II << "\t" << DETAIL << " // { " << *srcT << " " << countIndirections(srcT)
		...
		void processGEPOperand(CastInst *II, GetElementPtrInst *III, Type *innerSrcT, Type *innerDstT) {
			TheState.ClassifyPointerVariable(II->getOperand(0), UnifiedMemSafe::VariableStates::Seq);
			TheState.ClassifyPointerVariable(II, UnifiedMemSafe::VariableStates::Dyn);
		...
		Type *findRealDstType(CastInst *II, Type *innerDstT) {
			...
			for (Use &use : II->uses()) {
				...
				if (StoreInst *SI = dyn_cast_or_null<StoreInst>(userInst)) {
					...
					UMS_DEBUG(DEBUG_SWITCH, errs() << "LoadCast: Found Store Inst After Cast!\n"
						<< *II << "\t" << *SI << "\t" << *realDstType << "\n";);
					break;
					...
					UMS_DEBUG(DEBUG_SWITCH, errs() << "LoadCast: Found Cast Inst After Cast!\n" << *II << "\n" << *CI << "\n";);
					break;
				}
			}
		...
		Type *findLoadDstType(LoadInst *LI, CastInst *II) {
			...
			for (Use &LIuse : LI->uses()) {
				...
				if (CastInst *CII = dyn_cast_or_null<CastInst>(LIuserInst)) {
					...
					UMS_DEBUG(DEBUG_SWITCH, errs() << "LoadCast: Found Cast Inst for After Load Cast!\n" << *II << "\n" << *CII << "\n";);
					break;
				}
			}
			UMS_DEBUG(DEBUG_SWITCH, errs() << "LoadCast: Found Load Inst After Cast!\n" << *II << "\n" << *LI << "\n";);
			return realDstType;
		}
		...
		void logDowncastAnalysis(CastInst *II, Type *realSrcType, Type *realDstType, Type *innerSrcT, Type *innerDstT) {
			for (Type::subtype_iterator STI = realSrcType->subtype_begin(),
				...
				if ((llvm::Type *)STI == realDstType) {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "Load Downcast: " << *II << " From: " << *realSrcType << " To: " << *realDstType << "\n";);
					return;
				}
			}

			UMS_DEBUG(DEBUG_SWITCH, errs() << "No Load Downcast: " << *II << " From: " << *realSrcType << " To: " << *realDstType << "\n";);

			if (realSrcType->isIntegerTy() && innerSrcT->isIntegerTy() &&
				(!realDstType->isIntegerTy() || !innerDstT->isIntegerTy())) {
				UMS_DEBUG(DEBUG_SWITCH, errs() << "Potential Downcast: " << *II << " From: " << *realSrcType << " To: " << *realDstType << "\n";);
			}
		}
		...
		void propagateSizeMetadata(CastInst *II, UnifiedMemSafe::VariableInfo *varinfo) {
			if (varinfo != 0x0) {
				...
				UMS_DEBUG(DEBUG_SWITCH, errs() << "!!! DON'T KNOW variable or doesn't have size\n";);
			}
		}
		...
		void classifyExtOrTruncInst(Instruction *I) {
			...
			// Log the instruction
			UMS_DEBUG(DEBUG_SWITCH, errs() << "(EXTENSION/TRUNCATION) " << *I << "\n";);
			...
			if (ZExtInst *ZExt = dyn_cast<ZExtInst>(I)) {
				...
				UMS_DEBUG(DEBUG_SWITCH, errs() << "Handling SExtInst: " << *SExt << "\n";);
				...
				UMS_DEBUG(DEBUG_SWITCH, errs() << "Handling TruncInst: " << *Trunc << "\n";);
				...
				if (srcType->isIntegerTy() && dstType->isIntegerTy()) {
					...
					if (srcBitWidth > dstBitWidth) {
						...
						UMS_DEBUG(DEBUG_SWITCH, errs() << "Operand of TruncInst: " << *operand << "\n";);

						if (auto *loadInst = dyn_cast<LoadInst>(operand)) {
							Value *pointerOperand = loadInst->getPointerOperand();
							UMS_DEBUG(DEBUG_SWITCH, errs() << "Pointer operand of LoadInst: " << *pointerOperand << "\n";);

							for (User *user : pointerOperand->users()) {
								if (auto *storeInst = dyn_cast<StoreInst>(user)) {
									Value *storedValue = storeInst->getValueOperand();
									UMS_DEBUG(DEBUG_SWITCH, errs() << "Found StoreInst: " << *storeInst << "\n";);
									UMS_DEBUG(DEBUG_SWITCH, errs() << "Stored value: " << *storedValue << "\n";);
							...
							UMS_DEBUG(DEBUG_SWITCH, errs() << "Operand is neither a LoadInst nor a constant, assuming potential signedness change.\n";);
							couldChangeValue = true;
						}
					}
				}
		...
		bool processInstruction(Instruction *I) {
			...
			if (AllocaInst *II = dyn_cast_or_null<AllocaInst>(I)) {
    			...
    			UMS_DEBUG(DEBUG_SWITCH, errs() << "(R) " << *RI << "\n";);
				...
				UMS_DEBUG(DEBUG_SWITCH, errs() << DETAIL << " " << *I << NORMAL << "\n";);
			}
		...
		void analyzeFunction(Function *F){
			...
			if (isCurrentFunctionWhitelisted){
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\t[whitelisted]\n";);
			}
			if (isCurrentFunctionWhitelistedForInstrumentation){
				UMS_DEBUG(DEBUG_SWITCH, errs() << "\t[whitelisted for instrumentation]\n";);
			}
		...
		void classificationPropagation(Module &M, pdg::PTAWrapper &ptaw, pdg::ProgramGraph *pdgraph)
		{
			...
			for (auto it = TheState.Variables.begin(); it != TheState.Variables.end(); it++) {
				if (it->second.isGlobal && it->second.didClassificationChange) {
					...
					for (auto &depFuncName : it->second.dependentFunctions) {
						...
						UMS_DEBUG(DEBUG_SWITCH, errs() << depFuncName << " ";);
					}
					UMS_DEBUG(DEBUG_SWITCH, errs() << "\n";);
						...
						for (inst_iterator i = inst_begin(*F), e = inst_end(*F); i != e; ++i) {
							...
							for (Use &U : (&*i)->operands()) {
								...
								if (it->first == dyn_cast<GlobalVariable>(U)) {
									auto *node = pdgraph->getNode(*i);
									if (!node) 
										continue;

									auto &aliasSet = node->getInNeighborsWithDepType(pdg::EdgeType::DATA_ALIAS);
									for (auto &alias : aliasSet) {
										auto aliasInst = dyn_cast<Instruction>(alias->getValue());
										if (aliasInst) {
											UMS_DEBUG(DEBUG_SWITCH, errs() << "Found alias instruction: " << *aliasInst << "\n";);
											processInstruction(aliasInst);
										}
									}

									// inefficient, just get the neighbors from PDG
									...
									// 		if (!(ptaw._ander_pta)||
									// 			!(ptaw.hasValueNode(*InstGlobalOperand)) || 
									// 			!(ptaw.hasValueNode(*InstReAnalyze)))
									// 			continue;
		...
		void getAnalysisUsage(AnalysisUsage &AU) const override{
			...
			AU.addRequired<OutOfWindowPass>();

			// Preserve the analysis results
			AU.setPreservesCFG();
			AU.addPreserved<UnifiedMemSafePass>();
			AU.setPreservesAll();
		...
		bool runOnModule(Module &M) override{
			...
			ptaw = &pdg::PTAWrapper::getInstance();
			...
			errs() << "Registering globals...\n";
			registerGlobals(M);

			// 2) Collect functions to analyze
			errs() << "Collecting functions to analyze...\n";
			...
			errs() << "Collect Function Time: " << (t1 - start_time) << " ms\n";

			// 3) Analyze them
			errs() << "Analyzing functions...\n";
			analyzeAllFunctions(FunctionsToAnalyze);

			auto t2 = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
			errs() << "Analyze Function Time: " << (t2 - t1) << " ms\n";

			UMS_DEBUG(DEBUG_SWITCH, errs() << "-------------------------------------------------\n";);
			for (auto it = TheState.Variables.begin(); it != TheState.Variables.end(); ++it) {
				if (it->second.classification == VariableStates::Dyn) {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "DYN";);
				} else if (it->second.classification == VariableStates::Seq) {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "SEQ";);
				} else if (it->second.classification == VariableStates::Safe) {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "SAFE";);
				} else {
					UMS_DEBUG(DEBUG_SWITCH, errs() << "Other";);
				}
				UMS_DEBUG(DEBUG_SWITCH, errs() << " -- " << *it->first << "\n";);
			}
			UMS_DEBUG(DEBUG_SWITCH, errs() << "-------------------------------------------------\n";);

			// 4) Classification propagation
			errs() << "Propagating classifications...\n";
			errs() << "Size of variable map before propagation: " << TheState.Variables.size() << "\n";
			classificationPropagation(M, *ptaw, pdgraph);

			auto t3 = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
			errs() << "Classification Propagation Time: " << (t3 - t2) << " ms\n";
			...
			// Get the out of window analysis results
			OutOfWindowPass &OOWPass = getAnalysis<OutOfWindowPass>();
			std::set<const Instruction *> &oowResults = OOWPass.getOOWInstructions();
			errs() << "Out of Window Analysis Results:\n" << oowResults.size() << " instructions found.\n";
			...
			errs() << "Identify Unsafe Heap Pointers Time: " << (t4 - t3) << " ms\n";
			...
			errs() << "Identify Unsafe Stack Pointers Time: " << (t5 - t4) << " ms\n";
				        ...
				        for (auto &I : BB) {
				            if (auto *call = dyn_cast<CallInst>(&I)) {
				                ...
				                if (auto *asmVal = llvm::dyn_cast<llvm::InlineAsm>(calledVal)) {
									...
									if (SFIOnly && (asmStr == MAGIC_ASM_BEGIN || asmStr == MAGIC_ASM_END)) {
										toBeRemoved.push_back(call);
									}

				                    if (asmStr == NORMAL_MAGIC_ASM_BEGIN || asmStr == NORMAL_MAGIC_ASM_END) {
				                        toBeRemoved.push_back(call);
				                    }
				                }
				            }
			...
			errs() << "Time taken: " << (end_time - start_time) << " ms\n";
