...
#define SLH_DEBUG_SWITCH 0
#define SLH_DEBUG(S, C)                                                        \
  do {                                                                         \
    if (S)                                                                     \
      C                                                                        \
  } while (0)
...
STATISTIC(NumAddrsHardened, "Number of address mode addresses hardaned");
...
STATISTIC(NumCMOVInserted, "Number of cmove instructions inserted");
STATISTIC(NumInstInsertedForJMPandCALL, "Number of instructions inserted for jumps and calls");
STATISTIC(NumInstInsertedAllOne, "Number of instructions inserted to move -1 to a register");
STATISTIC(NumInstInsertedForEFLAGs, "Number of instructions inserted for saving and restoring EFLAGS");
STATISTIC(NumInstInsertedForLOADAndSTORE, "Number of instructions inserted for load and store");
STATISTIC(NumInstInsertedForConditions, "Number of instructions inserted for conditions");
STATISTIC(NumInstInsertedForVTInsts, "Number of instructions inserted for variable timing instructions");

STATISTIC(NumInstMarkedByASLH, "Number of instructions marked by ASLH as to be hardened");
STATISTIC(NumLoadOrStoreMarkedByASLH, "Number of loads or store marked by ASLH as to be hardened");
STATISTIC(NumDataguardProtected, "Number of positives reported by dataguard get protected");
STATISTIC(NumStandAloneGEP, "Number of or inserted for stand alone geps");
STATISTIC(RemainingAnalysisPositives, "Number of analysis positives not hardened");
STATISTIC(NumCmovFiltered, "Number of CMOV instructions filtered out");

STATISTIC(NumHardenedBasicBlocks, "Number of basic blocks hardened");
STATISTIC(NumHardenedFunctions, "Number of functions hardened");

STATISTIC(NumTotalBasicBlocks, "Number of basic blocks in the binary");
STATISTIC(NumTotalFunctions, "Number of functions in the binary");
...
static cl::opt<bool> EnableFilter(
    PASS_KEY "-filter",
    cl::desc("Filter out speculative safe load."),
    cl::init(false), cl::Hidden);

static cl::opt<bool> EnableStoreFilter(
  PASS_KEY "-filter-store",
  cl::desc("Filter out speculative safe store."),
  cl::init(false), cl::Hidden);

static cl::opt<bool> HardenStandAloneGEPs(
  PASS_KEY "-geps",
  cl::desc("Harden stand alone GEPs."),
  cl::init(true), cl::Hidden);

static cl::opt<bool> EnableCMOVFilter(
  PASS_KEY "-filter-cmov",
  cl::desc("Remove unused CMOVs"),
  cl::init(false), cl::Hidden);

static cl::opt<bool> RemoveAllHardening(
  PASS_KEY "-remove-harden",
  cl::desc("Not hardening anything but keeps the cmovs"),
  cl::init(false), cl::Hidden);
...
public:
  ...
  static char *DUMMY_BEFORE;
  static char *DUMMY_AFTER;
...
private:
  ...
  bool isPrefixedByDummies(MachineInstr &MI);
  void markAsProtected(MachineInstr &MI, SmallPtrSet<MachineInstr *, 1> &DummyInstrHardenedBySLH);
  ...
  unsigned hardenStandAloneGEP(MachineInstr &MI);
...
char *X86SpeculativeLoadHardeningPass::DUMMY_BEFORE = "addq 123456, %rax";
char *X86SpeculativeLoadHardeningPass::DUMMY_AFTER = "addq 654321, %rax";
...
bool X86SpeculativeLoadHardeningPass::runOnMachineFunction(
  ...
  // as we go.
  if (!RemoveAllHardening)
    tracePredStateThroughBlocksAndHarden(MF);

  // SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Number of CMOVs before removing: " << CMovs.size() << "\n";);
  // MF.dump();
  // // remove unused cmovs
  // if (EnableCMOVFilter) {
  //   for (MachineInstr *CMovI : CMovs) {
  //     for (MachineOperand &op : CMovI->defs()) {
  //       bool used = false;
  //       for (MachineBasicBlock &MBB : MF){
  //         for (MachineInstr &MI : MBB) {
  //           if (CMovI != &MI) {
  //             for (MachineOperand &MO : MI.uses()) {
  //               if (MO.isReg() && MO.getReg() == op.getReg()) {
  //                 used = true;
  //                 break;
  //               }
  //             }
  ...
  //       if (!used) {
  //         ++NumCmovFiltered;
  //         CMovs.erase(std::remove(CMovs.begin(), CMovs.end(), CMovI), CMovs.end());
  //         CMovI->eraseFromParent();
  //       }
  //     }
  //   }
  // }
  // MF.dump();
  // SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Number of CMOVs after removing: " << CMovs.size() << "\n";);
  ...
  // SLH_DEBUG(SLH_DEBUG_SWITCH, MF.dump(););
...
void X86SpeculativeLoadHardeningPass::tracePredStateThroughBlocksAndHarden(
  ...
  SmallPtrSet<MachineInstr *, 1> DummyInstr;
  SmallPtrSet<MachineInstr *, 1> StandAloneGEPDummyInstr;
  SmallPtrSet<MachineInstr *, 1> DummyInstrHardenedBySLH;
  SmallPtrSet<MachineInstr *, 1> HardenStandAloneGEP;
...
//  SmallPtrSet<MachineInstr *, 1> HardenFixedAddressRIP;
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    if (HardenLoads)
      for (MachineInstr &MI : MBB) {
        SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "-----------\n";);
        SLH_DEBUG(SLH_DEBUG_SWITCH, MI.dump(););
        for (MachineOperand &Op : MI.uses()){
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "USE: ";);
          SLH_DEBUG(SLH_DEBUG_SWITCH, Op.dump(););
        }
        for (MachineOperand &Op : MI.defs()){
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "DEF: ";);
          SLH_DEBUG(SLH_DEBUG_SWITCH, Op.dump(););
        }
        SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "EFLAGS is " << (isEFLAGSLive(MBB, MI, *TRI) ? "live" : "dead") << "\n";);

        if (MI.isInlineAsm()) {
          const char *inlinedAsmBefore = MI.getOperand(0).getSymbolName();
          if (strcmp(inlinedAsmBefore, DUMMY_BEFORE) == 0) {
            SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Found dummyBefore: ";);
            SLH_DEBUG(SLH_DEBUG_SWITCH, MI.dump(););
            MachineInstr *last_MI = nullptr;
            MachineInstr *MI_next = MI.getNextNode();
            int instCountBetween = 0;
            while (MI_next) {
              if (MI_next->isInlineAsm()) {
                const char *inlinedAsmAfter = MI_next->getOperand(0).getSymbolName();
                if (strcmp(inlinedAsmAfter, DUMMY_AFTER) == 0) {
                  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Found dummyAfter: ";);
                  SLH_DEBUG(SLH_DEBUG_SWITCH, MI_next->dump(););
                  break;
                }
              }
              instCountBetween++;
              // We need to harden everything between the dummyBefore and dummyAfter
              SLH_DEBUG(SLH_DEBUG_SWITCH, MI_next->dump(););
              if (!MI_next->isDebugInstr()) {
                last_MI = MI_next;
              }
              MI_next = MI_next->getNextNode();
            }
            
            if (instCountBetween > 0) {
              if (last_MI != nullptr) {
                HardenStandAloneGEP.insert(last_MI);
                if (instCountBetween != 2) {
                  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "!!!Not 2 instructions between dummyBefore and dummyAfter\n";);
                }
                SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Found StandAloneGEP, will harden:";);
                SLH_DEBUG(SLH_DEBUG_SWITCH, last_MI->dump(););
              }
              StandAloneGEPDummyInstr.insert(&MI);
              StandAloneGEPDummyInstr.insert(MI_next);
            } else {
              SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Found regular dummys\n";);
              DummyInstr.insert(&MI);
              DummyInstr.insert(MI_next);
            }
          }
        }
        ...
        if ((BaseReg && LoadDepRegs.test(BaseReg)) ||
            (IndexReg && LoadDepRegs.test(IndexReg))) {
              SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Dependency (Base | Index): " << LoadDepRegs.test(BaseReg) << " | " << LoadDepRegs.test(IndexReg) << "\n";);
          continue;
        }
    ...
    if (EnableFilter) {
      if (DummyInstr.size() > 0 || StandAloneGEPDummyInstr.size() > 0) {
        errs() << "Hardened Basic Block: " << MF.getName() << "||" << MBB.getName() << "\n";
        ++NumHardenedBasicBlocks;
        anyHarden = true;
      } else {
        errs() << "Not Hardened Basic Block: " << MF.getName() << "||" << MBB.getName() << "\n";
      }
    } else {
      if (!HardenLoadAddr.empty()) {
        errs() << "Hardened Basic Block: " << MF.getName() << "||" << MBB.getName() << "\n";
        ++NumHardenedBasicBlocks;
        anyHarden = true;
      } else {
    ...
    // Harden all the instructions we have marked as stand-alone GEPs.
    // The reason this is a separate pass is that we need to remove all the 
    // dummys inserted for these stand-alone GEPs before going to the next
    // pass so that we do not over-harden loads.
    if (HardenStandAloneGEPs) {
      for (MachineInstr *MI : HardenStandAloneGEP) {
        SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Harden stand alone GEP: ";);
        SLH_DEBUG(SLH_DEBUG_SWITCH, MI->dump(););
        unsigned HardenedReg = hardenStandAloneGEP(*MI);
        AddrRegToHardenedReg[HardenedReg] = HardenedReg;      
      }
    }

    for (MachineInstr *MI : StandAloneGEPDummyInstr) {
      SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Remove stand alone GEP dummys\n";);
      MBB.erase(MI);
    }

    NumInstMarkedByASLH += HardenLoadAddr.size();
    
    for (MachineInstr *MI : HardenLoadAddr) {
      if (MI->mayLoad() || MI->mayStore()) {
        ++NumLoadOrStoreMarkedByASLH;
      } else {
        SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "neither load nor store: ";);
        SLH_DEBUG(SLH_DEBUG_SWITCH, MI->dump(););
      }
    }
    ...
    for (MachineInstr &MI : MBB) {
      SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "++++++++++++++++++++++++++\n";);
      SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Current Instruction: ";);
      SLH_DEBUG(SLH_DEBUG_SWITCH, MI.dump(););
      if (HardenLoads) {
        ...
        if (HardenLoadAddr.erase(&MI)) {
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "HardenLoadAddr\n";);
          
          bool inOtherHardenList = HardenCondition.contains(&MI) ||
                                   HardenFixedAddressRBP.contains(&MI) ||
                                   HardenFixedAddressRSP.contains(&MI) ||
                                   HardenVTInstr.contains(&MI);

          if (EnableFilter) {
            if ((!EnableStoreFilter &&
                 !HardenStoreAddr.contains(&MI) &&
                 !inOtherHardenList) ||
                 (EnableStoreFilter && !inOtherHardenList)) {
              if (!isPrefixedByDummies(MI)) {
                // add dummy
          ...
          markAsProtected(MI, DummyInstrHardenedBySLH);

          if (HardenStoreAddr.erase(&MI)) {
            SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "HardenStoreAddr\n";);
            hardenStore(MI);
            continue;
          }
        ...
        if (HardenPostLoad.erase(&MI)) {
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "HardenPostLoad\n";);
        ...
        if (HardenFixedAddressRBP.erase(&MI) || HardenFixedAddressRSP.erase(&MI)) {
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "HardenFixedAddress\n";);
          markAsProtected(MI, DummyInstrHardenedBySLH);
        ...
        if (HardenCondition.erase(&MI)) {
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "HardenCondition\n";);
          markAsProtected(MI, DummyInstrHardenedBySLH);
        ...
        if (HardenVTInstr.erase(&MI)) {
          SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "HardenVTInstr\n";);
    ...
    for (auto MI : DummyInstrHardenedBySLH) {

      SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Remove dummy hardened by SLH\n";);
      SLH_DEBUG(SLH_DEBUG_SWITCH, MI->dump(););
      DummyInstr.erase(MI);
      auto ret = MBB.erase(MI);
      SLH_DEBUG(SLH_DEBUG_SWITCH, ret->dump(););
    }

    RemainingAnalysisPositives += DummyInstr.size() / 2;
    // finally, remove all dummy instructions not touched
    for (auto MI : DummyInstr) {
      SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Remove regular dummys\n";);
      MBB.erase(MI);
    }
  
    SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Instructions in MBB: " << MBB.size() << "\n";);
    SLH_DEBUG(SLH_DEBUG_SWITCH, errs() <<"====================\n";);
    for (MachineInstr &MI : MBB) {
      SLH_DEBUG(SLH_DEBUG_SWITCH, MI.dump(););
      for (MachineOperand &Op : MI.uses()){
    ...
    DummyInstr.clear();
    StandAloneGEPDummyInstr.clear();
    HardenStandAloneGEP.clear();
    DummyInstrHardenedBySLH.clear();
    hardened_RSP = false;
  ...
  if (anyHarden) {
    ++NumHardenedFunctions;
    errs() << "Hardened function: " << MF.getName() << "\n";
  } else {
    errs() << "Not Hardened function: " << MF.getName() << "\n";
  }
}

bool X86SpeculativeLoadHardeningPass::isPrefixedByDummies(MachineInstr &MI) {
  MachineInstr *prevMI = MI.getPrevNode();
  MachineInstr *prevPrevMI = prevMI ? prevMI->getPrevNode() : nullptr;
  if (prevMI != nullptr) {
    SLH_DEBUG(SLH_DEBUG_SWITCH, prevMI->dump(););
    SLH_DEBUG(SLH_DEBUG_SWITCH, MI.dump(););
  }

  if (!(prevMI && prevMI->isInlineAsm())) {
    SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "No inlineAsm, continue\n";);
    return false;
  }

  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "InlineAsm\n";);
  const char *inlinedAsm = prevMI->getOperand(0).getSymbolName();
  if (strcmp(inlinedAsm, DUMMY_AFTER) != 0) {
    SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Wrong inlined asm, continue\n";);
    return false;
  }
...
void X86SpeculativeLoadHardeningPass::markAsProtected(MachineInstr &MI, SmallPtrSet<MachineInstr *, 1> &DummyInstrHardenedBySLH) {
  if (!isPrefixedByDummies(MI)) {
    return;
  }
  ...
  MachineInstr *prevMI = MI.getPrevNode();
  MachineInstr *prevPrevMI = prevMI ? prevMI->getPrevNode() : nullptr;
  if (DummyInstrHardenedBySLH.contains(prevMI)) {
    SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Already marked hardened\n";);
  }

  DummyInstrHardenedBySLH.insert(prevMI);
  DummyInstrHardenedBySLH.insert(prevPrevMI);
  NumDataguardProtected++;
}
...
unsigned X86SpeculativeLoadHardeningPass::saveEFLAGS(
  ...
  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "saveEFLAGS to reg: " << Reg << "\n";);
...
void X86SpeculativeLoadHardeningPass::restoreEFLAGS(
    ...
    Register Reg) {
  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "restoreEFLAGS from reg: " << Reg << "\n";);
...
unsigned X86SpeculativeLoadHardeningPass::hardenStandAloneGEP(MachineInstr &MI) {
  ++NumStandAloneGEP;
  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "Harden StandAloneGEP :::::\n";);
  MachineBasicBlock &MBB = *MI.getParent();
  const DebugLoc &Loc = MI.getDebugLoc();
  ...
  unsigned HardenedReg = hardenValueInRegister(
      OldDefReg, MBB, std::next(MI.getIterator()), Loc);

  MachineInstr *HardeningInst = MRI->getVRegDef(HardenedReg);

  // MRI->replaceRegWith(/*FromReg*/ OldDefReg, /*ToReg*/ HardenedReg);
  for (auto &U : make_early_inc_range(MRI->use_operands(OldDefReg))) {
    MachineInstr *UserInst = U.getParent();
    if (UserInst == HardeningInst) {
      U.setReg(UnhardenedReg);
      continue;
    }
    U.setReg(HardenedReg);
  }
  return HardenedReg;
}
...
void X86SpeculativeLoadHardeningPass::hardenLoadAddr(
  ...
  int oldNumAddrRegsHardened = NumAddrRegsHardened;
  for (MachineOperand *Op : HardenOpRegs) {
    ...
    if (!Subtarget->hasVLX() && (OpRC->hasSuperClassEq(&X86::VR128RegClass) ||
      ...
      (void)MovI;
      ++NumInstsInserted;
      ++NumInstInsertedForLOADAndSTORE;
  ...
  if (oldNumAddrRegsHardened < NumAddrRegsHardened) 
    ++NumAddrsHardened;
...
unsigned X86SpeculativeLoadHardeningPass::hardenValueInCondition(
  ...
  SLH_DEBUG(SLH_DEBUG_SWITCH, errs() << "hardenValueInCondition:" << OpIdx << "\n";);
  SLH_DEBUG(SLH_DEBUG_SWITCH, MI.getOperand(OpIdx).dump(););
